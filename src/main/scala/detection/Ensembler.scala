...
class Ensembler(){
	...
	def detectAndCombine(eType:String, ensembleMode: String, threshold: Double, detectors: List[Detector]):DataFrame = {
		...
		combineAll(eType, ensembleMode, threshold, detected)
	}
	...
	def combineAll(eType: String, ensembleMode: String, threshold: Double, anomalies: Seq[DataFrame]):DataFrame = {
		val combined = anomalies.tail.foldLeft(anomalies.head){case (anoms1, anoms2) => 
			combine2(eType, ensembleMode, threshold, anoms1, anoms2)}
	...
	private def combine2(eType: String, ensembleMode: String, threshold: Double,
		...
		val scoreColName = ensembleMode+"_"+anom1ScoreCol+"_"+anom2ScoreCol
